...
impl AttachableUsbDevice for RodeCasterProIIDevice {
    ...
    fn get_state(&self) -> Result<Structured> {
        let Ok(state) = self.state.lock() else {
            return Err(anyhow!(
                "Failed to acquire lock on device state. Cannot get state."
            ));
        };
        Ok(state.clone())
    }
            ...
            match read_framed_message(|| {
                ...
                Ok(packet) => match packet {
                    RodeCasterPacketResult::Unknown(data) => {
                        warn!("Received unknown RODECaster packet {:?}", data)
                    }
                    RodeCasterPacketResult::PropertyUpdate(update) => {
                        let Ok(mut state) = state.lock() else {
                            error!(
                                "Failed to acquire lock on device state. Cannot apply property update."
                            );
                            continue;
                        };
                    ...
                    RodeCasterPacketResult::DeviceReport(report) => {
                        let Ok(mut state) = state.lock() else {
                            error!(
                                "Failed to acquire lock on device state. Cannot apply device report update."
                            );
                            continue;
                        };
